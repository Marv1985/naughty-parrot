...
function frontend_scripts() {
    ...
    wp_enqueue_style('frontendstyles', get_template_directory_uri() . '/style.css', false, filemtime(get_template_directory() . '/style.css'), 'all');


    // Enqueue Scripts
    wp_enqueue_script('gsap', 'https://cdn.jsdelivr.net/npm/gsap@3.12.7/dist/gsap.min.js');
    wp_enqueue_script('scrollTrigger', 'https://cdn.jsdelivr.net/npm/gsap@3.12.7/dist/ScrollTrigger.min.js', array('gsap'), null, true);
...
function backend_scripts() {
   ...
   // Enqueue Scripts
   wp_enqueue_script('admin-scripts', get_template_directory_uri() . '/js/wordpressJS/admin_custom.js', array('wp-blocks', 'wp-element', 'wp-hooks'), '', true); //FOR IMAGE HOVER PREVIEW WHEN SELECTING BLOCK
}
...
add_action('enqueue_block_editor_assets', 'backend_scripts');
...
\*------------------------------*/
function filter_block_categories( $categories, $editor_context ) {
    // Define your custom block category
    $custom_block_category = array(
        'slug'  => 'custom-block-category',
        'title' => 'Custom Blocks',
    );

    // Reset categories to only include your custom block category
    $categories = array( $custom_block_category );

    return $categories;
}

// Apply the filter
add_filter( 'block_categories', 'filter_block_categories', 10, 2 );
